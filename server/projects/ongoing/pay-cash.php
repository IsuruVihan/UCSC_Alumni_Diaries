<?php
    ...
    $query10 = "SELECT Amount FROM projectcash WHERE ProjectId='{$ProjectId}'";
    $query11 = "SELECT SpentAmount FROM projectcashspendings WHERE Id='{$RequestId}'";
    $query = "UPDATE projectcashspendings SET Status='Paid' WHERE Id='{$RequestId}'";
    $query2 = "
    ...
    while ($row10 = mysqli_fetch_assoc($results10)) {
        while ($row11 = mysqli_fetch_assoc($results11)) {
            $Balance = $row10['Amount'] - $row11['SpentAmount'];
            if ($Balance >= 0) {
