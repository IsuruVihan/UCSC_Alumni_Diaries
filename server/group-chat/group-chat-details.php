 ...
 include('../../db/db-conn.php');
    ...
    $query ="SELECT * FROM groupchats WHERE Id = '{$Id}'";
    $result = mysqli_query($conn, $query);

    if ($row = mysqli_fetch_assoc($result)) {
        echo "
            <form class='row-01' id='edit-group-{$row['Id']}' enctype='multipart/form-data' >
            ...
            "; 
                        ...
                        if ($row['PicSrc'] == 'group-chat.png') {
                            echo "
                                <img src='../../assets/images/group-chat.png' width='13%' height='90%' class='user-pic-02' alt='user-pic'>
                            "; 
                        }else{
                            echo "
                                <img src='../../uploads/group-chat/{$row['PicSrc']}' width='15%' height='90%'  class='user-pic-02' alt='user-pic'>
                            
                                ";
                        } 
        echo "
                  <form id='editgroup-{$row['Id']}' enctype='multipart/form-data' >
                        <input type='file' id='edit-file-{$row['Id']}' name='files[]'  >
                        <input type='text' class='new-project-name input-field' placeholder='Enter new Group name'  id='Newgroup-name-{$row['Id']}' name='Newgroup-name'/>
                        <button class='edit-btn btn' id='edit-{$row['Id']}' onclick=submitEdit('{$row['Id']}') >Edit</button>
                        <input type='button' value='Cancel' class='cancel-btn btn' onclick='HideEditProjectNameDiv()'>
                   </form>
                    </div>
                    </div>
            </form>
