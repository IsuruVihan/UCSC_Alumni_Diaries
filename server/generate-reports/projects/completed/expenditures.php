<?php
    ...
    $query2 = "
        SELECT ItemId, ItemName, SpentQuantity, Description, Timestamp FROM projectitemspendings
    ...
    $pdf->Cell(207, 30, 'Completed project - Expenditures report', 0, 0, 'C');
    ...
    while ($row = mysqli_fetch_assoc($results)) {
        ...
        $pdf->Write(25, "\n", '', 0, 'C', true, 0, false, false, 0);
        $pdf->SetFont('Arial','BU',12);
        $pdf->Cell(20, 10, 'Items Spendings');
        $pdf->Write(15, "\n", '', 0, 'C', true, 0, false, false, 0);
    
        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(15,10, 'Item Id',1,0,'C');
        $pdf->Cell(40,10, 'Item Name',1,0,'C');
        $pdf->Cell(20,10, 'Spent Qty',1,0,'C');
        ...
        if (mysqli_num_rows($results2) > 0) {
            while ($row2 = mysqli_fetch_assoc($results2)) {
                $pdf->Cell(15,10, $row2['ItemId'],1,0,'C');
                $pdf->Cell(40,10, $row2['ItemName'],1,0,'C');
                $pdf->Cell(20,10, $row2['SpentQuantity'],1,0,'C');
                $pdf->Cell(80,10, $row2['Description'],1,0,'C');
                $pdf->Cell(35,10, $row2['Timestamp'],1,0,'C');
                $pdf->Ln();
            }
        } else {
            $pdf->Cell(190,10, "No data",1,0,'C');
    ...
    $query4 = "
        ...
        VALUES ('{$_SESSION['Email']}', 'Projects - Completed', 'Expenditures of Project (ID): {$ProjectId} - Report generated')
    ";
